...
function displayCourses(courseSelection) {
  ...
  const totalCredits = courseSelection.reduce((total, course) => total + course.credits, 0);
  creditTotal.textContent = `Total credits displayed: ${totalCredits}`;
}
...
filterButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const filteredCourses =
      filter === "all" ? courses : courses.filter((course) => course.subject === filter);
